...
class Pointer(nn.Module):
    def __init__(self, d_model):
        ...
        self.w3 = Initialized_Conv1d(d_model * 2, 1)
        self.w4 = Initialized_Conv1d(2, 1, bias=True)
    ...
    def forward(self, M1, M2, M3, mask):
        ...
        Z1 = self.w3(X1 * F.softmax(Y1))

        Z2 = torch.cat([Z1, Y2], dim=1)
        Z2 = self.w4(Z2)

        Y1 = mask_logits(Y1.squeeze(), mask)
        Z3 = mask_logits(Z2.squeeze(), mask)
        return Y1, Z3
...
class QANet(nn.Module):
    def __init__(self, word_mat, char_mat,
                 ...
                 dropout=0.1, num_head=8):  # !!! notice: set it to be a config parameter later.
        super().__init__()
        if train_cemb:
